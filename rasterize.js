        ...
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].textureCoor.push(uvToAdd[0], 1 - uvToAdd[1]);

                    //console.log(inputTriangles[whichSet].textureCoor);
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
                    ...
                    //console.log(ellipsoidModel.textures);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            if(uLightMode == 1)
              gl_FragColor = vec4(fragmentColor.rgb, fragmentColor.a * uAlpha);  //replace lighting
            else
              gl_FragColor = vec4(fragmentColor.rgb * colorOut, fragmentColor.a * uAlpha); //modulation lighting
         // gl_FragColor = fragmentColor; //no lighting
        }
    `;
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        if(currSet.material.alpha < 1) {
          ...
          sortedArray.push({depth: getDepth(center4[2]), index: whichTriSet});
        }
    } // end for each triangle set
    // render each ellipsoid
    var ellipsoid, instanceTransform = mat4.create(); // the current ellipsoid and material
    for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
        ...
        if(ellipsoid.alpha < 1) {
          ...
          var center4 = vec4.fromValues(center[0], center[1], center[2], 1.0);
          mat4.multiply(center4, mMatrix, center4);
          sortedArray.push({depth: getDepth(center4[2]), index: whichEllipsoid + numTriangleSets});
        }
    } // end for each ellipsoid

    for(var index = 0; index < sortedArray.length; index ++) {
      if(index < numTriangleSets) {
        currSet = inputTriangles[index];
        makeModelTransform(currSet);
        mat4.multiply(pvmMatrix,pvMatrix,mMatrix); // project * view * model
        gl.uniformMatrix4fv(mMatrixULoc, false, mMatrix); // pass in the m matrix
        gl.uniformMatrix4fv(pvmMatrixULoc, false, pvmMatrix); // pass in the hpvm matrix
        ...
        ellipsoid = inputEllipsoids[index- numTriangleSets+1];
        makeModelTransform(ellipsoid);
        pvmMatrix = mat4.multiply(pvmMatrix,pvMatrix,mMatrix); // premultiply with pv matrix
        gl.uniformMatrix4fv(mMatrixULoc, false, mMatrix); // pass in model matrix
        gl.uniformMatrix4fv(pvmMatrixULoc, false, pvmMatrix); // pass in project view model matrix
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[index+1]); // activate vertex buffer
        gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed vertex buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[index+1]); // activate normal buffer
        gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed normal buffer to shader
        gl.bindBuffer(gl.ARRAY_BUFFER, textureBuffers[index+1]);
        gl.vertexAttribPointer(vTextAttribLoc,2,gl.FLOAT,false,0,0);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, textures[index+1]);
        gl.uniform1i(samplerUniform, 0);
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        gl.enable(gl.BLEND);
        gl.disable(gl.DEPTH_TEST);
        gl.depthMask(false);
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, triangleBuffers[index+1]); // activate tri buffer
        // draw a transformed instance of the ellipsoid
        gl.drawElements(gl.TRIANGLES, triSetSizes[index+1], gl.UNSIGNED_SHORT,0); // render
      }
    }
} // end render model

function setupTexture() {
    var path = "https://ncsucgclass.github.io/prog3/";
    for(var whichTriSet = 0; whichTriSet < numTriangleSets; whichTriSet++) {
        initTexture(whichTriSet);
        textures[whichTriSet].image.src = path + inputTriangles[whichTriSet].material.texture;
    }
    for (var whichEllipsoid = 0; whichEllipsoid < numEllipsoids; whichEllipsoid++) {
        var index = numTriangleSets + whichEllipsoid;
        initTexture(index);
        textures[index].image.src = path + inputEllipsoids[whichEllipsoid].texture;
    }
}

function getDepth(zSet) {
  return Math.abs(zSet - Eye[2]);
}
...
function initTexture(index){
  ...
  textures[index].image.onload = function() {
      gl.bindTexture(gl.TEXTURE_2D, textures[index]);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, textures[index].image);
